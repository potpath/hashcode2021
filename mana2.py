...
def sol(D,I,S,V,F,streets,cars,from_ins,to_ins):
    car_ws = []
    for car in cars:
        suml = sum(streets[x][3] for x in car)
        car_ws.append(1./suml)

    in_street_count = [defaultdict(int) for _ in range(I)]
    street_ws = [0] * S
    for icar, car in enumerate(cars):
        for istreet in car:
            street_ws[istreet] += car_ws[icar]
...
def write(schs,D,I,S,V,F,streets,cars,from_ins,to_ins):
    ...
    for iin, sch in enumerate(schs):
        ...
        if not sch:
            ...
            continue
        print(len(sch))
        mind = min(x[1] for x in sch)
        for sid, duration in sch:
            print(streets[sid][2], max(1,min(D,int(duration/mind))))
